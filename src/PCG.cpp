...
char PCG::TileMap::GetTileChar(PCG::TileType _tileType) const {
    ...
    case PCG::TileType::TILE_TYPE_Wall:
        return PCG::Wall_CHAR;
    default:
        return '?';
    }
}
...
void PCG::TileMap::LoadMapData(const char* _filename) {
    ...
    for (int y = 0; y < PCG::MAP_ROWS; y++) {
        for (int x = 0; x < PCG::MAP_COLUMNS; x++) {
            ...
            else if (ch == PCG::Wall_CHAR) {
                tileArray[y][x] = PCG::TileType::TILE_TYPE_Wall;
            }
        }
    }
...
void PCG::Room::CreateRoom() {
    int minDimention = TILE_SIZE * 2;
